...
namespace VKalVrtAthena {
  ...
  StatusCode VrtSecFuzzy::initialize()
  {
    ...
    if( m_jp.doSelectTracksFromMuons )     { m_trackSelectionAlgs.emplace_back( &VrtSecFuzzy::selectTracksFromMuons );     }
    if( m_jp.doSelectTracksFromElectrons ) { m_trackSelectionAlgs.emplace_back( &VrtSecFuzzy::selectTracksFromElectrons ); }
    ...
    if( !m_jp.doSelectTracksFromMuons && !m_jp.doSelectTracksFromElectrons ) {
      ...
      m_trackSelectionAlgs.emplace_back( &VrtSecFuzzy::selectTracksInDet );
...
/*    
    ...
    if( m_jp.doMergeByShuffling ) {
      m_vertexingAlgorithms.emplace_back( std::pair<std::string, vertexingAlg>( "mergeByShuffling",           &VrtSecFuzzy::mergeByShuffling )                   );
    }
      
    if ( m_jp.doMergeFinalVerticesDistance ) {
      m_vertexingAlgorithms.emplace_back( std::pair<std::string, vertexingAlg>( "mergeFinalVertices",          &VrtSecFuzzy::mergeFinalVertices )                );
    }
    
    if( m_jp.doAssociateNonSelectedTracks ) {
      m_vertexingAlgorithms.emplace_back( std::pair<std::string, vertexingAlg>( "associateNonSelectedTracks",  &VrtSecFuzzy::associateNonSelectedTracks )        );
    }
...
*/
    ...
    if( m_jp.FillHist ) {
      ...
      std::string histDir("/AANT/VrtSecFuzzy" + m_jp.augVerString + "/");
    ...
    if( m_jp.FillNtuple ) {
      ...
      m_tree_Vert = new TTree("tree_VrtSecFuzzy","TTree of VrtSecFuzzy"); 
      ATH_CHECK( hist_root->regTree("/AANT/tree_VrtSecFuzzy", m_tree_Vert) );
...
//--------------------------------------------------------
  ...
  StatusCode VrtSecFuzzy::execute()
  {
    ...
    ATH_CHECK( evtStore()->record( secondaryVertexContainer,    "VrtSecFuzzy_" + m_jp.secondaryVerticesContainerName + m_jp.augVerString          ) );
    ATH_CHECK( evtStore()->record( secondaryVertexAuxContainer, "VrtSecFuzzy_" + m_jp.secondaryVerticesContainerName + m_jp.augVerString + "Aux." ) );
    ...
    if( m_jp.FillIntermediateVertices ) {
      ...
      ATH_CHECK( evtStore()->record( twoTrksVertexContainer,      "VrtSecFuzzy_" + m_jp.all2trksVerticesContainerName + m_jp.augVerString          ) );
      ATH_CHECK( evtStore()->record( twoTrksVertexAuxContainer,   "VrtSecFuzzy_" + m_jp.all2trksVerticesContainerName + m_jp.augVerString + "Aux."  ) );
      ...
      for( auto itr = m_vertexingAlgorithms.begin(); itr!=m_vertexingAlgorithms.end(); ++itr ) {
        ...
        ATH_CHECK( evtStore()->record( intermediateVertexContainer,      "VrtSecFuzzy_IntermediateVertices_" + name + m_jp.augVerString           ) );
        ATH_CHECK( evtStore()->record( intermediateVertexAuxContainer,   "VrtSecFuzzy_IntermediateVertices_" + name + m_jp.augVerString + "Aux."  ) );
      }
